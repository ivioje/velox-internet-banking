  ...
  return (
    ...
    <SheetContent side={'left'} className='border-none bg-white'>
        <Link href="/" className='cursor-pointer items-center gap-1 flex'>
            <Image src='/icons/logo.svg' width={34} height={34} alt='Velox Logo' />
            <h1 className='text-26 font-ibm-plex-serif font-bold text-black-1'>Velox</h1>
        </Link>
                ...
                sidebarLinks.map((items) => {
                    ...
                    <SheetClose asChild key={items.route}>
                        <Link href={items.route} key={items.label} className={cn('mobilenav-sheet_close w-full', {'bg-bank-gradient': isActive})}>
                            <Image src={items.imgURL} alt={items.label} className={cn({'brightness-[3] invert-0': isActive})} width={20} height={20} />
